...
import threading, time, ctypes, random, argparse, sys, math
import colorsys
...
parser.add_argument("--image", type=str, help="Set an image.")
# Python 3.9+
...
def worker_video(freq, amp, phase, running_event):
    ...
    async def main():
        ...
        all_auto_modes = ("flower", "flower2", "flowermix", "dots", "dots2", "bricks", "wall", "konfetti", "konfetti_wall", "triangle", "triangle_colorstrobe", "dope")
            ...
            match args.effect:
                ...
                case "flower2"|"flower2_dummy":
                    ...
                    if a>b:
                        ...
                        b=a
                        a=t
                    x = limit(random.randint(a,b)%8,3,7)
                    for i in range(int(x)):
                        ...
                        fc = flower_size // 2
                        # Blume zeichnen
                        maximal = int((flower_size*math.pi)/((PETAL_SIZE*2)+13))
                    ...
                    if state["flip"]:
                        time *= -1
                        rotation *= -1
                case "dots2"|"dots2_dummy":
                    hint= hint or f"Dots 2"
                    PETAL_RADIUS = 1 # int(HEIGHT/3)
                    flower_size=HEIGHT
                    a=int(freq.value)
                    b=int(amp.value)
                    if a>b:
                ...
                case "dots"|"dots_dummy":
                    ...
                    PETAL_RADIUS = int(HEIGHT/2)
                    flower_size=HEIGHT
                    x = limit(random.randint(a,b)%8,1,7)
                    for i in range(int(x)):
                        PETAL_SIZE = 15
                        maximal = int((flower_size*math.pi)/((PETAL_SIZE*2)+13))
                        minimum = int(((amp.value + freq.value) % 1 ) * 50)
